...
class ferramental_requisicao_model extends MY_Model {
	...
	public function query($id_obra = null, $id_usuario = null){
		...
		//$this->dd($this->user->id_usuario, $this->user->nivel);

		if($this->user->nivel==2)
		{
			$query->group_start();
			$query->where("solicitante.nivel", 2)
			->or_where('destino.id_obra', $this->user->id_obra)
			->or_where('origem.id_obra', $this->user->id_obra);
			$query->group_end();
